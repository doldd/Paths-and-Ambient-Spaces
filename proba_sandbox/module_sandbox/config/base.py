...
class BaseConfig:
    ...
    def __post_init__(self):
        ...
        for field in dataclasses.fields(self):
            if inspect.isclass(field.type):  # Simple Enum Annotation
                if issubclass(field.type, enum.Enum):  # Parse to Enum Types
                    self._modify_field(
                        **{field.name: field.type(getattr(self, field.name))}
                    )
            else:  # Complex Enum Annotation
                value = parse_complex(getattr(self, field.name), field.type)
                self._modify_field(**{field.name: value})
...
def parse_complex(attr, annot: Annotation):
    """
    Parse the complex type hints recursively.

    Notes
    ----------------
    - Currently focuses on parsing annotations for Enum types.
    """
    if attr is None:
        return
    ...
    if isinstance(annot, (GenericAlias, typing._GenericAlias)):
        if issubclass(typing.get_origin(annot), typing.List):
            if not isinstance(attr, typing.List):
                return attr
            return [
                parse_complex(element, typing.get_args(annot)[0]) for element in attr
            ]
            ...
            if not isinstance(attr, typing.Dict):
                return attr
            return {
                parse_complex(key, typing.get_args(annot)[0]): parse_complex(
    ...
    if isinstance(annot, typing.ForwardRef):
        return parse_complex(attr, eval(annot.__forward_arg__))
