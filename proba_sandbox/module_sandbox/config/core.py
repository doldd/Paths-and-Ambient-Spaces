"""Top level configuration class for the module_sandbox package."""
import dataclasses
import importlib
import inspect
import json
...
from module_sandbox.config.base import BaseConfig
from module_sandbox.config.data import DataConfig
from module_sandbox.config.models.base import ModelConfig
from module_sandbox.config.training import TrainingConfig
from module_sandbox.exceptions import MissingConfigError, ModelNotFoundError

models_module = importlib.import_module('module_sandbox.models')
...
class Config(BaseConfig):
    ...
    Notes
    ------
    - This is the top level configuration class for the module_sandbox package.
    ...
    def to_template(cls, model: str | nn.Module):
        ...
        ----------
            model: str | nn.Module
                ...
                in the `__init__.py` file of `module_sandbox.models` submodule,
                it can be referenced by a string name. Otherwise, pass the model
                class directly to generate the template.

        Notes
        -----
        - Each Machine Learning Model in the `module_sandbox` franemwork must have a
    ...
    def list_avaliable_models():
        ...
        - Models must be defined in the __init__.py file of
            `module_sandbox.models` submodule.
        """
    ...
    def _get_model_config_cls(model: nn.Module) -> type[ModelConfig]:
        ...
        - Each Machine Learning Model in the `module_sandbox` franemwork has a
            `config` field with corresponding `ModelConfig` subclass as its type.
